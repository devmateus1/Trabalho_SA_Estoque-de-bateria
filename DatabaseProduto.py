...
class DataBase:
    ...
    def RegistrarNoBanco(self, idProduto, tipo, voltagem, marca, quantidade, preco, data):
        self.cursor.execute("INSERT INTO usuario (idProduto, tipo, voltagem, marca, quantidade, preco, data) VALUES (%s, %s ,%s ,%s ,%s, %s, %s)",
                            (idProduto,tipo, voltagem, marca, quantidade, preco, data)) # Insere os dados do usuario na tabela
        self.conn.commit() # Confirma a inseção dos dados

    # Metodo para alterar os dados de um usuario existente no banco de dados
    def alterar (self,idProduto, tipo, voltagem, marca, quantidade, preco, data):
        self.conn.execute("UPDATE usuario SET tipo=%s, voltagem=%s, ,marca=%s, quantidade=%s, preco=%s, data=%s WHERE idProduto=%s",(tipo, voltagem, marca, quantidade, preco, data, idProduto)) # Atualiza os dados do usuario com o id fornecido
    ...
    def excluir (self, idProduto) :
        self.cursor.execute ("DELETE FROM usuario WHERE idProduto = %s", (idProduto,)) # Excluir o usuario com idusuario
        self.conn.commit() # Confirma a exclusão dos dados

    # Metodo para buscar os dados de um usuario no banco de dados
    def buscar(self, idProduto):
        self.cursor.execute("SELECT * FROM usuario WHERE idProduto= %s", (idProduto,)) # Seleciona os dados do usuario com o id fornecido
